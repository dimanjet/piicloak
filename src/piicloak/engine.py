...
from .config import DETECTOR_BACKEND, SPACY_MODEL, DEFAULT_LANGUAGE
from .privacy_filter import create_privacy_filter_analyzer
...
def create_detector_backend(name: str = DETECTOR_BACKEND, nlp=None):
    """Create the configured detector backend."""
    normalized = name.strip().lower().replace("_", "-")
    if normalized == "presidio":
        return create_analyzer(nlp)
    if normalized in {"privacy-filter", "opf"}:
        return create_privacy_filter_analyzer()
    raise ValueError("Unsupported detector backend. Use 'presidio' or 'privacy-filter'.")
